...
docker exec worldr-postgres pg_dump --schema-only -d migrated -U mmuser > $DUMPDIR/migrated.sql
docker exec worldr-postgres pg_dump --schema-only -d latest -U mmuser > $DUMPDIR/latest.sql

echo "Removing databases created for db comparison"
docker exec worldr-postgres sh -c 'exec echo "DROP DATABASE migrated; DROP DATABASE latest;" | exec psql -U mmuser mattermost_test'
...
if [ $diffErrorCode -eq 0 ]; then
    ...
    cat $DUMPDIR/migrated.sql > ./m.sql
    cat $DUMPDIR/latest.sql > ./l.sql
fi
