.PHONY: build package run stop run-client run-server start-server stop-client stop-server restart restart-server restart-client start-docker clean-dist clean nuke check-style check-client-style check-server-style check-unit-tests test dist prepare-enteprise run-client-tests setup-run-client-tests cleanup-run-client-tests test-client build-linux build-osx build-windows internal-test-web-client vet run-server-for-web-client-tests diff-config prepackaged-plugins prepackaged-binaries test-server test-server-quick test-server-race
...
app-iface: ## Extract interface from App struct
	env GO111MODULE=off $(GO) get gopkg.in/reflog/struct2interface.v0
	$(GOPATH)/bin/struct2interface.v0 -f "app" -o "app/app_iface.go" -p "app" -s "App" -i "AppIface" -t ./app/layer_generators/app_iface.go.tmpl
	$(GO) run ./app/layer_generators -in ./app/app_iface.go -out ./app/opentracing_layer.go -template ./app/layer_generators/opentracing_layer.go.tmpl
...
store-mocks: ## Creates mock files.
	# The mock code is generated by mockery library.
	# Get the binary at https://github.com/vektra/mockery/releases/tag/v1.1.0
	# and place it in server/bin.
	$(GOBIN)/mockery -dir store -all -output store/storetest/mocks -note 'Regenerate this file using `make store-mocks`.'
	$(GOBIN)/mockery -dir store/sqlstore -name IVault -output store/sqlstore/mocks -note 'Regenerate this file using `make store-mocks`.'
...
vet: ## Run mattermost go vet specific checks
	...
	$(GO) vet -vettool=$(GOBIN)/mattermost-govet -structuredLogging -inconsistentReceiverName -tFatal ./...
ifeq ($(BUILD_ENTERPRISE_READY),true)
ifneq ($(MM_NO_ENTERPRISE_LINT),true)
	# $(GO) vet -vettool=$(GOBIN)/mattermost-govet -enterpriseLicense -structuredLogging -tFatal ./enterprise/...
	$(GO) vet -vettool=$(GOBIN)/mattermost-govet -structuredLogging -tFatal ./enterprise/...
endif
endif
...
## Help documentating à la https://marmelab.com/blog/2016/02/29/auto-documented-makefile.html
help:
